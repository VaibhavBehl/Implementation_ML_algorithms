function do_linear_ridge_regression(allData, trainPercent, splitRuns, kFolds, lambda)
...
for split=1:splitRuns
    ...
    for lambdaValue = lambda
        ...
        for j=1:kFolds % for all k values, j will be the test set
            ...
            mse = linear_regression_find_test_error(crossTrainX,crossTrainY,crossTestX,crossTestY,lambdaValue);
            sumMse = sumMse + mse;
        end
    ...
    err(split) = linear_regression_find_test_error(trainX,trainY,testX,testY,bestLambda);
end
